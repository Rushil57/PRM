...
public partial class estimateview_popup : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            if (!string.IsNullOrEmpty(statementIDFromQueryString) || !string.IsNullOrEmpty(estimateIDFromQueryString))
            {
                ...
                #endif
                var cmdParams = new Dictionary<string, object> { { paramName, paramName == "@EstimateID" ? estimateIDFromQueryString : statementIDFromQueryString }, { "@StatementDate", DateTime.Now } };
            ...
            else
            {
                ...
                var cmdParams = new Dictionary<string, object> { { param, id }, { "@StatementDate", DateTime.Now } };
                ViewState["RequestFrom"] = param == "@EstimateID" ? "Estimate" : "Statment";
                AssignSpValuesToProperties(cmdParams);
            }

        }

        hdnShowMessagePopup.Value = "0";
    }
    ...
    private void AssignSpValuesToProperties(object cmdParams)
    {
        try
        {
            var reader = SqlHelper.ExecuteDataTableProcedureParams("web_pr_estimate_build", cmdParams as Dictionary<string, object>);
            ...
            foreach (DataRow row in reader.Rows)
            {
                ...
                if (ClientSession.PracticeID != 0)
                {
                    ...
                    {
                        //RadWindow.RadConfirm("Error Message will be there ", "closeRadWindow", 330, 180, null, "Message");
                        hdnShowMessagePopup.Value = "1";
                        return;
                    }
                }
                ...
                PracticeAddr4 = row["PracticeAddr3"].ToString();
