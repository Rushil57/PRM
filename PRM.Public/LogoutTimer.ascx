    ...
    function sm_timer(sessionLength, redirectUrl) {
        ...
        function updateDisplay() {
            if (displayElements != null) {
                ...
                var s = timeleft % 60;
                document.getElementById("sm_logoutTimer").innerHTML = "Are you still working? To protect your privacy, your session will expire soon. <br/> Auto logout in: " + m.toString() + ':' + (s <= 9 ? '0' : '') + s.toString();
            }
        }
        ...
        function checkPrompt() {
            if (timeleft <= 120 && !modalVisible) {
                modalVisible = true;
                var radWindow = $find("<%=windowLogoutManager.ClientID %>");
                radWindow.radconfirm('', logout_callback, 400, 140, null, '', 'Content/Images/warning.png');
                displayElements = $('u');
            }
        }
        function logout() {
...
<telerik:RadWindowManager ID="windowLogoutManager" Behaviors="Move" Style="z-index: 200001"
        ...
        <div class="rwDialogPopup radconfirm" style="width: 300px;">
            ...
            <div style="margin: 15px 0 0 30px;">
                <a href="#" onclick="$find('{0}').close(true);">
                    <img src="Content/Images/btn_continue_small.gif" alt="Continue" /></a> &nbsp; &nbsp;
...
<asp:Button ID="btnLogOut" Text="Log Out" Style="display: none;" OnClick="btnLogout_Click" runat="server" />
